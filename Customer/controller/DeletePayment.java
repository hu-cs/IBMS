	...
	private boolean delete() {
		...
		if (selectedrow != -1) {
			...
			int ask = new JOptionPane().showConfirmDialog(null,
					CustomersList.AreYouSure, CustomersList.Alert, JOptionPane.YES_NO_OPTION,
					JOptionPane.YES_OPTION);

			if (ask == JOptionPane.YES_OPTION) {

				String date = giveingAccountTable.getValueAt(selectedrow, 0).toString();
				float cost = Float.parseFloat(giveingAccountTable.getValueAt(selectedrow, 1).toString());
			...
			new JOptionPane().showMessageDialog(null,
					CustomersList.PleaseSelectOneRow , CustomersList.Information,JOptionPane.INFORMATION_MESSAGE);
