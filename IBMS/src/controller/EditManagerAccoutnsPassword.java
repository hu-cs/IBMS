...
public class EditManagerAccoutnsPassword implements ActionListener {

	JPasswordField lastPassword;
	JPasswordField newPassword;
	JDialog dialog;

	public EditManagerAccoutnsPassword(JPasswordField lastPassword,
	...
	private void changePassword() {
		...
		if (lastPassword.getText().equalsIgnoreCase(
			...
			updateAccountList();
			new JOptionPane().showMessageDialog(null, "انجام شد!");
			dialog.dispose();
	...
	private int validator() {
		...
		if (lastPassword.getText().trim().isEmpty()) {
			new JOptionPane().showMessageDialog(null,
					"لطفا کلمه عبور قبلی را وارد کنید");
			return -1;
