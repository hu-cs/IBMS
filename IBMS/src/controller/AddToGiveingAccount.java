...
public class AddToGiveingAccount implements ActionListener {
	...
	public int validator() {
		try {
			...
			float cost = Float.parseFloat(AddAccoutn.getValueAt(0, 1)
					.toString());
		} catch (NullPointerException e) {
			JOptionPane.showMessageDialog(null,
					CustomersList.PleaseEnterAllInformation,
					CustomersList.Information, JOptionPane.INFORMATION_MESSAGE);

			return -1;
		} catch (NumberFormatException e) {
			JOptionPane.showMessageDialog(null,
					CustomersList.PleaseInsertCorrectInformation,
					CustomersList.Information, JOptionPane.INFORMATION_MESSAGE);
	...
	private void insert(JTable customerTable) {
		...
		try {
			PreparedStatement statement = DBConnection.connection
					.prepareStatement("INSERT INTO customer_giving_account (Name,Cost,Date_And_Time, datayearid) VALUES ('"
							+ name
							+ "',"
							+ cost
							+ ",'"
