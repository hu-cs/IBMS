...
public class AddingConfirmBtn implements ActionListener {
	...
	private int save() {
		...
		try {
			...
			// "نام محصول قبلآ در سیستم ثبت شده است !");
			JOptionPane.showMessageDialog(null,
					"نام محصول قبلآ در سیستم ثبت شده است !", " هشدار  ",
	...
	private void insertIntoSoldMaterailList() {
		String name = table.getValueAt(0, 3).toString();
		String unit = table.getValueAt(0, 2).toString();
		try {

			java.sql.PreparedStatement statemnet = DBConnection.connection
					.prepareStatement("insert into sold_material_list (name,unit,quantity,datayearid) values('"
							+ name + "','" + unit + "',0," + datayearid + ")");
