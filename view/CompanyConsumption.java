...
public class CompanyConsumption extends JDialog {
	...
	public CompanyConsumption() {
		...
		deleteTypeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				new DeleteConsumptionTypeUi();//TODO problem
			}
		});
	...
	private void updateCombobox(JComboBox<String> typeCombo) {
		...
		try {
			PreparedStatement statement = (PreparedStatement) DBConnection.connection
					.prepareStatement("SELECT distinct Title FROM `companyconsumptions` where dataYearid = "
	...
	private void setTableValues() {
		...
		try {
			PreparedStatement statement = (PreparedStatement) DBConnection.connection
					.prepareStatement("SELECT title,Invoice_Number,Invoice_Cost,Date_And_Time FROM `companyconsumptions` WHERE Invoice_Cost>0 and datayearid="
							+ DataYearId.getDataYearId());
			ResultSet result = statement.executeQuery();
			while (result.next()) {
